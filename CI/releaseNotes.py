...
def allPulls(releaseDate):
    ...
    baseurl = "https://api.github.com/repos/swagger-api/swagger-converter/pulls/"
    content = ghApiClient.readUrl('repos/swagger-api/swagger-converter/pulls?state=closed&base=master&per_page=100')
    for l in content:
...
def lastReleaseDate(tag):
    content = ghApiClient.readUrl('repos/swagger-api/swagger-converter/releases/tags/' + tag)
...
def addRelease(release_title, tag, content):
    ...
    content = ghApiClient.postUrl('repos/swagger-api/swagger-converter/releases', payload)
    return content
